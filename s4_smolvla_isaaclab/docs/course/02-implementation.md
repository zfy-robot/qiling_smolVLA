...
::: info 本节要点
本章以 `drawer_insert_close` 为贯穿案例，看理论如何一步步落成可跑的砖石——脚本化专家、HDF5 数据、LeRobotDataset、SmolVLA 训练与在线闭环控制，环环相扣。一切以当前配置与代码为准，命令均从 `run.sh` 的真实入口出发。
:::
...
### 6.2.2.3 Observation 与 action 对齐
...
### 6.2.3.3 Episode、frame 与 timestamp
...
### 6.2.4.5 Fresh training 与 Resume
...
### 6.2.5.3 Chunk 重规划与融合

模型每次生成 50 帧，但默认执行到第 30 个策略帧（1.5 s）时重新观测并生成新 chunk。重叠区对新旧动作交叉融合，阶段切换还额外平滑任务文本改变造成的动作跳变。

```text
policy frame  0                         29 30   34 35 39
Chunk A       [===============================]
Chunk B                                      [===============================]
融合区                                        <---5 frames--->

阶段切换     old task |<------5-frame transition------>| new task
```

重规划较快提高响应性，但增加推理次数和随机 chunk 之间的不连续；重规划较慢减少计算，却让机器人在环境变化后继续执行旧预测。当前 30 帧是工程默认，不是 SmolVLA 理论规定的唯一值。

### 6.2.5.4 20 Hz 动作到 120 Hz 控制
...
### 6.2.5.5 Raw、Fused、Masked、Command 与 Actual
...
- 当前轻量门控保护明显的执行延迟，只把“抽屉未有效拉开”作为阶段级硬失败，其余偏差留给后续动作和最终物理状态判定。

### 6.2.5.7 `complete` 与 `success`
