...
| 固定项 | 随机项（当前默认） |
|---|---|
| 机器人基座和柜体布局 | 抽屉初始开度（`drawer_initial_open.enabled=true`） |
| 相机内外参 | |
| 工作室光照 | |
| 主罐名义位姿与尺寸（`can_xy.enabled=false`） | |
| 不生成三个干扰物（`distractor_cans.enabled=false`） | |
| 抓取后固定抬升偏移、专家控制逻辑 | |

YAML 仍保留已验证的主罐 5×5 分层网格范围与干扰物区域，作为以后把对应
`enabled` 改回 `true` 的配方。采集可用 `--can-xy-randomization` /
`--distractor-cans` 临时打开。

固定光照减少了不必要的视觉分布宽度，有利于先解决操作策略。

## 3.4 主罐连续随机区域（可选配方，当前关闭）
...
当前任务通过固定光照、三路相机、可选的主罐分层格内随机、抽屉初始开度和可选的
远离抓取区干扰物构造数据变化。当前默认关闭主罐 XY 与干扰物，优先把接触操作跑稳；
YAML 仍保留分层网格配方。运动学可达只是必要条件，物理抓取稳定性仍需在仿真中验证。
...
固定主罐、不生成干扰物，主要随机抽屉初开度。多轮共用同一 RNG 流。
