...
ORIGINAL_PATH="$PATH"
SMOLVLA_PREFIX="${SMOLVLA_PREFIX:-/home/zfy/miniconda3/envs/smolvla}"
...
CAMERA_WIDTH=680
CAMERA_HEIGHT=480
TRAIN_CONFIG="configs/smolvla_s4_bimanual.yaml"
TRAIN_STEPS=""
BATCH_SIZE=""
SAVE_FREQ=""
CLEAN_FIRST=false
...
Training options:
  ...
  --config PATH             Training config. Default: configs/smolvla_s4_bimanual.yaml
...
echo "[PIPELINE]   record stages call run.sh record-hdf5, which switches child process to env_isaaclab via IsaacLab/isaaclab.sh"
...
if [[ "$SKIP_RECORD" != true ]]; then
    mkdir -p "$OUTPUT_DIR"
    if (( WORKERS > 1 )); then
        echo "[PIPELINE] recording HDF5 with $WORKERS direct record-hdf5 workers"
...
if [[ "$SKIP_TRAIN" != true ]]; then
    ...
    bash run.sh train-smolvla "${TRAIN_ARGS[@]}"
fi
