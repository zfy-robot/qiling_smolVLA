...
10. 红色任务物体已从红色圆柱替换为 `s4_smolvla_isaaclab/assets/scenes/Pill_Bottle.usdz`。为了不破坏现有 `--block red|blue` 控制接口、HDF5 字段和历史脚本，prim 路径仍保留 `/World/RecordTask/RedBlock`，HDF5 键仍保留 `states/rigid_object/red_block/root_pose`。当前训练任务仍只抓取蓝色圆柱；红色瓶子固定不随机。
...
4. 桌面任务支撑物已从三个独立垫块改成一个固定大垫块 `task_platform`。红色瓶子、蓝色圆柱和盘子都在同一个固定平台上。
5. 泛化采集默认只随机蓝色圆柱初始位置，x/y 方向独立采样 `uniform(-0.03m, +0.03m)`。红色瓶子、垫块和盘子固定不动。
...
118. 2026-07-31 按用户要求把红色圆柱替换为瓶子资产：`s4_robot/simulation.py` 新增 `COCA_COLA_BOTTLE_USDZ = assets/scenes/Coca_Cola_Bottle.usdz`。第一次尝试直接用 `UsdFileCfg` 把 USDZ 作为 `RigidObject`，但用户运行时在加载桌子后立即退出，说明 USDZ 作为动态刚体/碰撞资产不够稳。当前修正为更稳的代理结构：`/World/RecordTask/RedBlock` 仍使用原来的 `CylinderCfg` 作为不可见碰撞代理，完整沿用蓝色圆柱同一套质量 `BLOCK_MASS=0.08kg`、刚体 solver/damping、collision contact/rest offset 和摩擦材质；随后隐藏 `/World/RecordTask/RedBlock/geometry/mesh`，并把 `Coca_Cola_Bottle.usdz` 挂到 `/World/RecordTask/RedBlock/BottleVisual` 作为可见子资产。注意：为兼容现有控制、采集和转换脚本，prim/HDF5 历史命名仍是 `RedBlock/red_block`，但实际可视资产已是 Coca-Cola bottle。若瓶子在可视化中悬空或陷入平台，优先调整 `BOTTLE_VISUAL_Z_OFFSET`；若尺寸不合适，优先调整 `BOTTLE_SCALE`。
119. 2026-07-31 用户反馈代理方案后仍然闪退。加 `[BOOT]` 和 fatal traceback 后定位到根因：`simulation.py` 里的 `PROJECT_ROOT = Path(__file__).resolve().parents[2]` 指到了外层 `/home/zfy/smolVLA`，导致瓶子路径被解析成不存在的 `/home/zfy/smolVLA/assets/scenes/Coca_Cola_Bottle.usdz`；实际资产在 `/home/zfy/smolVLA/s4_smolvla_isaaclab/assets/scenes/Coca_Cola_Bottle.usdz`。已修正为 `PROJECT_ROOT = Path(__file__).resolve().parents[1]`。随后用户验证场景可加载但仍显示红色圆柱，日志显示 `set_prim_visibility() got an unexpected keyword argument 'stage'`，原因是 IsaacLab 5.1 的 `set_prim_visibility` 签名为 `(prim, visible)`，不是 `(path, visible, stage=...)`。已修正为先取 `/World/RecordTask/RedBlock/geometry/mesh` prim，再调用 `set_prim_visibility(collision_mesh_prim, False)`；瓶子 visual 创建不再被这个签名错误跳过。同时保留启动阶段 `[BOOT]` 日志和 traceback，`run.sh sim` 默认追加 `--continuous`，用于后续场景加载问题快速定位。
120. 2026-07-31 按用户新要求彻底去掉红色圆柱和 `Coca_Cola_Bottle.usdz`，改为直接加载同目录 `assets/scenes/Pill_Bottle.usdz`。实测该 USDZ 的 bbox 原始尺寸约为 `(119.62, 108.79, 67.42)`，`metersPerUnit=0.01`，`upAxis=Y`。第一次按 USD metadata 换算后使用 `PILL_BOTTLE_SCALE_VALUE=0.1103`，用户验证发现瓶子特别大，说明当前 Isaac 引用 USDZ 的链路没有按预期自动应用 `metersPerUnit=0.01`，不能再按“厘米已转米”来算 scale。已修正为直接按 raw authored bbox 高度 `108.79` 缩放到旧红色圆柱高度 `0.12m`，即 `PILL_BOTTLE_SCALE_VALUE=0.001103`，近似尺寸为 `(0.132, 0.120, 0.074)m`。为了把资产 Y-up 转成 Isaac Z-up，使用根姿态 `PILL_BOTTLE_ORIENTATION=(0.7071068, 0.7071068, 0, 0)`。`TaskLayout.red_block_pos()` 仍保留原来的 x/y 任务槽位 `(block_x, table_center_y + block_y_offset)`，z 改为平台表面附近 `surface_z + 0.005`，这样视觉中心约等于旧圆柱中心高度。`write_object_pose()` 新增可传入四元数，`reset_scene()` 重置红色瓶子时继续写入该 Y->Z 姿态，避免 reset 后瓶子倒回错误方向。为降低 USDZ 缺少物理 schema 的风险，新增 `configure_usdz_rigid_meshes()`：在 `/World/RecordTask/RedBlock` 根 prim 定义刚体/质量，在所有 Mesh 上定义 collision + convex hull collision，并绑定与蓝色圆柱一致的摩擦材质。当前蓝色圆柱抓取任务不使用红色物体动作，红色瓶子主要是固定场景物体/干扰物。
121. 2026-07-31 诊断 IsaacLab 资产浏览/加载慢的问题：项目代码和 `configs/s4_bimanual_dataset.json` 里的默认场景路径本身是本地路径，且本地资产根真实结构为 `/home/zfy/isaacsim_assets/Assets/Isaac/5.1/{Isaac,NVIDIA,...}`，所以 scene/table 路径中多出来的 `/Isaac/...` 是正确的。真正不一致在 IsaacLab 启动配置：`/home/zfy/IsaacLab/apps/isaaclab.python*.kit` 和 conda 包内 `.../isaacsim/kit/data/Kit/Isaac-Sim/5.1/user.config.json` 仍把 `/persistent/isaac/asset_root/default/cloud/nvidia` 指向 `https://omniverse-content-production.s3-us-west-2.amazonaws.com/Assets/Isaac/5.1`，而普通 GUI 的用户配置 `/home/zfy/.local/share/ov/data/Kit/Isaac-Sim Full/5.1/user.config.json` 已经有本地 default。已在项目 `run.sh` 中新增 `ISAAC_LOCAL_ASSET_ROOT=/home/zfy/isaacsim_assets/Assets/Isaac/5.1`，并对 `sim/record-hdf5/eval-smolvla/headless` 通过 `--kit_args` 覆盖 `/persistent/isaac/asset_root/default`、`cloud`、`nvidia` 到本地。以后从本项目启动 IsaacLab 时应优先走本地资产；如果手动打开 Isaac Sim 的 Asset Browser 仍显示云端目录，检查 UI cache `/home/zfy/.local/share/ov/data/documents/Kit/shared/omni.kit.browser.asset.cache.json` 或在内容浏览器里直接添加本地 folder。
122. 2026-07-31 用户提供 `omni.kit.browser.folder.core.models.folder_browser_data` 日志，确认慢点主要来自 NVIDIA Assets / Asset Browser 的云端缩略图和目录缓存，而不是本项目的 warehouse/table USD 加载。`omni.kit.browser.asset-1.3.12/config/extension.toml` 默认 folders 全部是 `https://omniverse-content-production.s3-us-west-2.amazonaws.com/Assets/...`，并使用共享缓存 `${shared_documents}/omni.kit.browser.asset.cache.json`，本机对应 `/home/zfy/.local/share/ov/data/documents/Kit/shared/omni.kit.browser.asset.cache.json`。已继续扩展 `run.sh` 的 `ISAAC_LOCAL_KIT_ARGS`：除了 `/persistent/isaac/asset_root/default/cloud/nvidia` 外，还把 `/exts/omni.kit.browser.asset/folders/0..7` 改成本地 `file:/home/zfy/isaacsim_assets/Assets/Isaac/5.1/Isaac`、`NVIDIA/Assets/ArchVis/{Commercial,Industrial,Residential}` 和 `NVIDIA/Assets/DigitalTwin/Assets/Warehouse/{Equipment,Safety,Shipping,Storage}`，并把 Asset Browser timeout 降到 1s、启动默认隐藏。注意：这个修正只保证通过本项目 `bash run.sh sim/record/eval/headless` 启动时优先走本地；如果手动打开 Isaac Sim GUI 并点 NVIDIA Assets 面板，旧缓存仍可能显示/访问云端，需要手动清理该 cache 或直接用 Content Browser/File picker 指向本地资产根。
123. 2026-07-31 用户再次提供 ForkliftB/ForkliftC 云端缩略图日志，进一步定位到真正仍走 S3 的不是通用 `omni.kit.browser.asset`，而是 IsaacSim 专用浏览器：`isaacsim.asset.browser/config/extension.toml` 和 `isaacsim.gui.content_browser/config/extension.toml` 都把 `/exts/.../folders` 写死为 `https://omniverse-content-production.s3-us-west-2.amazonaws.com/Assets/Isaac/5.1/Isaac/{Robots,Environments,IsaacLab,Materials,People,Props,Samples,Sensors}`。已继续补齐 `run.sh`：新增 `ISAAC_SIM_BROWSER_ARGS` 覆盖 `/exts/isaacsim.asset.browser/folders/0..7`，新增 `ISAAC_GUI_CONTENT_BROWSER_ARGS` 覆盖 `/exts/isaacsim.gui.content_browser/folders/0..7`，都指向本地 `/home/zfy/isaacsim_assets/Assets/Isaac/5.1/Isaac/...`，并把 timeout 设为 1s。已同步修改两个 IsaacSim 用户配置文件：`/home/zfy/.local/share/ov/data/Kit/Isaac-Sim Full/5.1/user.config.json` 和 conda 包内 `.../isaacsim/kit/data/Kit/Isaac-Sim/5.1/user.config.json`，使手动打开 GUI 时也优先显示本地 Isaac 资产分类。注意：已经打开的 IsaacSim 窗口不会动态刷新这些启动设置，必须完全关闭所有 IsaacSim 进程后重新打开。
124. 2026-07-31 用户再次反馈“还是慢”，但日志已经从 `https://...` 变成 `file:/home/zfy/isaacsim_assets/...`，说明云端问题已解决，剩余慢点是本地 Asset Browser 正在递归扫描大目录并校验 `.thumbs` 缩略图匹配。实测本地 `Isaac/IsaacLab` 三层内约 430 个目录，`Isaac/Robots` 三层内约 412 个目录；默认把 `Isaac/IsaacLab` 放进浏览器 folders 会导致大量本地缩略图 warning 和长时间扫描。已收窄三套浏览器默认目录：`isaacsim.asset.browser`、`isaacsim.gui.content_browser`、`omni.kit.browser.asset` 的 Isaac 侧目录只保留 `Isaac/Environments`、`Isaac/Props`、`Isaac/Robots`，不再扫描 `Isaac` 根目录和 `Isaac/IsaacLab`。同时给 `isaacsim.asset.browser` 设置 `data/hide_file_without_thumbnails=false`，减少必须依赖缩略图的过滤。若要挑 IsaacLab 自带机器人/环境，手动用 File picker/Content Browser 进入 `/home/zfy/isaacsim_assets/Assets/Isaac/5.1/Isaac/IsaacLab`，不要把它放在启动默认扫描目录里。
125. 2026-07-31 开始把项目从“单个蓝色圆柱任务脚本”整理成可复用多任务结构。新增 `tasks/base.py` 定义 `TaskModuleSpec` 和 `TaskDataContract`；新增 `tasks/right_blue_cylinder_plate.py` 作为当前已验证任务；新增 `tasks/drawer_insert_close.py` 作为下一个“拉开抽屉、放入物品、关上抽屉”任务占位。`tasks/__init__.py` 现在维护 `TASK_REGISTRY` 和 `get_task_spec()`。新增 `configs/tasks/`，把当前任务 dataset/training 配置拆成 `right_blue_cylinder_plate.dataset.json` 和 `right_blue_cylinder_plate.smolvla.yaml`，并预置 `drawer_insert_close.dataset.json`、`drawer_insert_close.smolvla.yaml` 模板。稳定 active 配置仍保留 `configs/s4_bimanual_dataset.json` 和 `configs/smolvla_s4_bimanual.yaml`，新增 `bash run.sh activate-task <task_id>` 把注册任务模板复制到 active 配置，新增 `bash run.sh list-tasks` 查看任务注册表。后续新增任务优先新增 `tasks/<task_id>.py` 与 `configs/tasks/<task_id>.*`，不要复制整套转换/训练代码。
126. 2026-07-31 整理脚本命名，降低数字脚本混乱。新增语义化入口：`scripts/record_dataset.py`、`convert_lerobot.py`、`eval_policy.py`、`preview_policy.py`、`visualize_policy.py`、`policy_server.py`、`record_parallel.py`、`clean_generated.py`、`inspect_project.py`、`pipeline_collect_convert_train.sh`。这些新入口当前通过 `_legacy_entry.py` 调用旧数字脚本，旧 `00/03/05/...` 文件暂时保留为兼容层，避免一次重构破坏已跑通的 recorder/eval。`run.sh` 已改为优先调用语义化脚本名；`06_eval_smolvla_in_isaaclab.py` 内部 policy server 路径也改为 `scripts/policy_server.py`。后续重构原则：先保持蓝色圆柱任务作为回归测试，再逐步把 `s4_robot/simulation.py` 和 `03_record_physics_dataset.py` 中的任务对象/状态机迁移进具体 task module。
127. 2026-07-31 修正多任务切换的关键配置断点：`scripts/03_record_physics_dataset.py` 和 `scripts/06_eval_smolvla_in_isaaclab.py` 原来仍从 `s4_robot/simulation.py` 的 `DEFAULT_SCENE_USD/DEFAULT_TABLE_USD` 取场景和桌子，导致 `activate-task drawer_insert_close` 后 recorder/eval 仍可能加载旧圆柱任务场景。已改为统一通过 `s4_pipeline.config.load_project_config(CONFIG_PATH)` 读取 active `configs/s4_bimanual_dataset.json` 里的 `scene.scene_usd`、`scene.table_usd`、`scene.table_top_z`。以后新增任务时，active config 是场景加载事实来源；不要在通用入口里硬编码任务场景。
128. 2026-07-31 按用户要求创建抽屉任务第一版场景。`tasks/drawer_insert_close_scene.py` 已从占位改为可加载场景 builder：复用第一个任务的 Simple Warehouse、PackingTable、S4 机器人和 DebugFrontCamera，不再加载旧任务的 task_platform/red/blue/plate；额外加载抽屉 `Isaac/Props/Sektion_Cabinet/sektion_cabinet_instanceable.usd`，并在桌面上摆放 3 个 NaturalBostonRound 瓶、3 个 WhitePacker 小药瓶、4 个 YCB 罐/瓶。`configs/tasks/drawer_insert_close.dataset.json` 已改为使用同一 warehouse/table 路径，不再指向不存在的 `assets/scenes/drawer_task_scene.usd`。`03_record_physics_dataset.py` 新增按 active task 的 `scene_builder` 动态 import，并为非 `right_blue_cylinder_plate` 任务提供静态预览循环；因此 `bash run.sh activate-task drawer_insert_close && bash run.sh sim --print-layout` 能先查看抽屉任务环境，但当前还没有抽屉 open/insert/close 状态机，也不能用于正式数据采集。
129. 2026-07-31 按用户反馈精简抽屉预览场景：`tasks/drawer_insert_close_scene.py` 不再加载 NaturalBostonRound 三个瓶子和 WhitePacker 三个小药瓶，只保留抽屉柜和 4 个 YCB 罐/瓶资产：tomato_soup_can、master_chef_can、mustard_bottle、bleach_cleanser。为避免同一 IsaacSim 会话中残留上一个任务，抽屉 scene builder 启动时会删除 `/World/RecordTask`、`/World/TaskPlatform`、`/World/RedBlock`、`/World/BlueBlock`、`/World/Plate` 等 legacy prim。`s4_robot/simulation.py` 的 PackingTable 清理逻辑也从只停用 `/World/TaskTableVisual/container_h20` 扩展为停用名字包含 `container/crate/corrugatedbox/box_` 的自带物料 prim，但保留桌子本体。
130. 2026-07-31 按用户要求调整抽屉预览场景 z 位置：`tasks/drawer_insert_close_scene.py` 新增显式常量 `DRAWER_POSITION_Z=0.70` 和 `TABLE_OBJECT_POSITION_Z=0.52`。抽屉柜 `/World/DrawerTask/DrawerCabinet` 的位置现在是 `(0.72, table_center_y, 0.70)`；4 个桌面 YCB 物体的 z 坐标固定为 `0.52`，不再跟随 `table_top_z + 0.025`。
131. 2026-07-31 用户反馈 YCB 物体 z 高度没有按 `0.52` 生效，并要求 4 个罐子绕 x 方向旋转 `-90deg`。已在 `tasks/drawer_insert_close_scene.py` 新增 `OBJECT_ROTATE_X_NEG_90_QUAT=(0.7071068,-0.7071068,0,0)`，4 个 YCB placement 都使用该 orientation。为避免 `UsdFileCfg.func(... translation=...)` 被资产内部 authored xform/instanceable 结构覆盖，新增 `_force_root_transform()`：spawn 完后对根 prim 清空 xform op order，并重新写入 translate/orient/scale，使 `/World/DrawerTask/Objects/*` 根 prim 的 z 明确为 `TABLE_OBJECT_POSITION_Z=0.52`。
132. 2026-07-31 用户运行抽屉场景时报错：`XformOp </World/DrawerTask/DrawerCabinet.xformOp:orient> has typeName 'quatd' ... requested PrecisionFloat`。根因是抽屉 USD 根 prim 已有 double precision `xformOp:orient`，而 `_force_root_transform()` 默认 `AddOrientOp()` 创建 float precision orient，USD 不允许同名 op 类型冲突。已修正为：对根 prim 先删除所有已有 `xformOp:*` 和 `xformOpOrder` 属性，再用 double precision 重新创建 translate/orient/scale，并用 `Gf.Quatd/Gf.Vec3d` 写值。另一个问题：上次 PackingTable 清理把 `/World/TaskTableVisual/SM_CratePacking_Table_A1` 桌子主体也当作 crate 清掉，导致旧桌子/物料清理过度。已把清理规则收窄：不再用通用 `crate` token，只清理 `container/corrugatedbox/box_` 和名字以 `SM_Crate_A` 开头的具体 crate 子物料，保留桌子主体。
133. 2026-07-31 用户已在 IsaacSim 中微调抽屉任务场景并保存为 `/home/zfy/smolVLA/s4_smolvla_isaaclab/assets/scenes/cabinet_task.usd`，要求任务场景直接加载这个 USD。已重写 `tasks/drawer_insert_close_scene.py`：不再逐个加载 warehouse/table/Sektion/YCB 资产，也不再在 Python 中设置抽屉或罐子位姿；scene builder 只清理旧任务残留 prim，引用 active config 的 `scene.scene_usd` 到 `/World/CabinetTaskScene`，再添加 S4 robot 和 DebugFrontCamera。`configs/tasks/drawer_insert_close.dataset.json` 和当前 active `configs/s4_bimanual_dataset.json` 的 `scene_usd` 已改为 `assets/scenes/cabinet_task.usd`，`table_usd` 改为 `null`，防止再次叠加旧桌子。
134. 2026-07-31 用户验证直接加载 `cabinet_task.usd` 后场景为空、只剩机器人。已回滚第 133 条的实现：`tasks/drawer_insert_close_scene.py` 恢复为显式加载 base warehouse、PackingTable、Sektion cabinet 和 4 个 YCB 罐/瓶；`configs/tasks/drawer_insert_close.dataset.json` 和 active `configs/s4_bimanual_dataset.json` 恢复为本地 warehouse/table 路径，`table_usd` 不再为 null。保留此前修复：不加载 NaturalBostonRound/WhitePacker 六个瓶子，保留 YCB x 方向 `-90deg` 和 z=0.52，保留 double precision root xform 强制写入，保留不误删 `SM_CratePacking_Table_A1` 桌子主体的清理规则。
135. 2026-07-31 用户要求抽屉任务不再加载以前的 PackingTable 和里面任何东西，且只保留 TomatoSoupCan，其他三个 YCB 罐/瓶也不加载。已把 `configs/tasks/drawer_insert_close.dataset.json` 和 active `configs/s4_bimanual_dataset.json` 的 `scene.table_usd` 改为 `null`，所以 `spawn_background_and_table()` 只加载 warehouse，不加载旧桌子。`tasks/drawer_insert_close_scene.py` 的 `YCB_OBJECTS` 只保留 `005_tomato_soup_can.usd`，`_object_placements()` 只返回 `TomatoSoupCan`，位置 z 仍为 `TABLE_OBJECT_POSITION_Z=0.52`，姿态仍绕 x 轴 `-90deg`。抽屉 scene builder 启动时也会清理 `/World/TaskTableVisual`，避免同一 IsaacSim 会话中的旧桌子残留。
136. 2026-07-31 用户运行抽屉预览后发现 TomatoSoupCan 视觉仍卡在桌面/柜体里，且抽屉打不开。日志显示 `CreateJoint - cannot create a joint between static bodies`，根因是 `_spawn_usd(... kinematic=True)` 给 Sektion cabinet 根资产覆盖了 `kinematic_enabled=True`，PhysX 把抽屉关节两端都当静态体。已修改 `_spawn_usd()`：`kinematic` 默认改为 `None`，`None` 时不写 `RigidBodyPropertiesCfg`，保留资产自带物理属性；抽屉和 TomatoSoupCan 都用 `kinematic=None` 加载。TomatoSoupCan 高度改为 bbox 底部对齐：新增 `_bbox_min_z()` 和 `_align_bbox_bottom_z()`，spawn 后计算世界 bbox 最小 z，再调整根 prim translate，使视觉/碰撞 bbox bottom 对齐到 `TOMATO_SOUP_CAN_BOTTOM_Z=0.52`，不再把 root z 误认为物体底部 z。
137. 2026-07-31 继续修正抽屉预览两个问题。第一，用户已要求不再加载旧桌子，抽屉根位置为 `z=0.70`，因此旧的 `TomatoSoupCan bottom_z=0.52` 会把罐子放进柜体/抽屉内部；`tasks/drawer_insert_close_scene.py` 现在会在加载 Sektion cabinet 后计算 `/World/DrawerTask/DrawerCabinet` 的世界 bbox 顶面，把 TomatoSoupCan 的 bbox bottom 自动对齐到 `drawer_top_z + 0.01m`，`0.52` 只作为 bbox 失败 fallback。第二，非当前圆柱任务的 static preview 不再在 GUI 模式持续 `sim.step()`，而是只 `simulation_app.update()` 保持界面刷新；这样避免一边推进 physics 一边在 UI 里改 drawer articulation joint，减少 `Updating joint local poses in articulations is not supported after simulation start` 这类警告。后续真正打开/关闭抽屉不要依赖 UI 手拖，而应在 `tasks/drawer_insert_close_controller.py` 中做程序化 drawer joint/handle 控制。
138. 2026-07-31 用户询问“怎样才能把抽屉打开”。结论：不能在属性面板里拖 joint local pose 来打开，IsaacSim 已经明确警告仿真启动后不支持这种操作；应给抽屉 prismatic/revolute joint 写 drive target 或后续由机器人抓手物理拉开。已在 `scripts/03_record_physics_dataset.py` 增加调试参数：`--drawer-open`、`--drawer-joint-filter`、`--drawer-target`、`--drawer-drive-stiffness`、`--drawer-drive-damping`、`--drawer-drive-max-force`。运行 `bash run.sh sim --print-layout --drawer-open --drawer-joint-filter drawer_top_joint --drawer-target 0.35` 会枚举 `/World/DrawerTask/DrawerCabinet` 下的 joint，给匹配 joint 应用 USD Physics Drive，然后推进 physics 预览抽屉运动；方向反了就用 `--drawer-target -0.35`。README 和 `docs/TASKS.md` 已同步该命令。后续任务控制器应复用“程序化 joint drive/handle interaction”，不要再让人工在 UI 改 joint transform。
139. 2026-07-31 用户运行 `--drawer-open --drawer-target -0.35` 后，joint 枚举和 drive 写入成功，但 PhysX 报错：`PxArticulationJointReducedCoordinate::setDriveTarget(): it is illegal to call this method if PxSceneFlag::eENABLE_DIRECT_GPU_API is enabled`。根因不是 joint 名或 drive target，而是当前 `SimulationCfg(device='cuda:0')` 启用了 PhysX direct GPU API，这条路径不允许运行时给 articulation joint 调 `setDriveTarget()`。IsaacLab 5.1 的 `PhysxCfg` 没有单独暴露 `enable_direct_gpu_api` 字段，GPU/CPU PhysX 由 `SimulationCfg.device` 决定。已修改 `03_record_physics_dataset.py`：当传入 `--drawer-open` 且 CLI device 是 cuda 时，自动把 `create_simulation_context()` 的 device 改成 `cpu`，并打印说明；这样只影响抽屉 drive 预览，不影响已跑通的蓝色圆柱 GPU 采集/训练/eval 链路。
140. 2026-07-31 用户需要实时查看手臂 TCP 在世界/base_link 下的坐标和姿态，用于调一个更适合拉开抽屉的手臂角度。已在 `scripts/03_record_physics_dataset.py` 新增 `--print-tcp-pose` 和 `--tcp-print-period`。TCP 计算不再依赖 reach controller：直接读取 `right_wrist_yaw_link` 的世界 pose，并叠加当前 wrist-frame TCP offset `DEFAULT_TCP_OFFSET_WRIST=(0,0,-0.10)m`。日志格式包含 `pos_w=(x,y,z)`、`quat_wxyz=(w,x,y,z)` 和 `rpy_deg=(roll,pitch,yaw)`，其中 rpy 使用 IsaacLab `euler_xyz_from_quat()`，只用于人工调试，后续代码里仍优先保存/使用四元数。命令示例：`bash run.sh sim --print-layout --print-tcp-pose --tcp-print-period 0.5`。README 和 `docs/TASKS.md` 已同步。
141. 2026-07-31 用户要求在抽屉任务中把左手 TCP、右手 TCP、抽屉上把手 frame 都用坐标系可视化，并询问如何通过界面实时调试手臂。已扩展 `TcpFrameVisualizer`：新增 `/World/Visuals/LeftHandTCP`、`/World/Visuals/RightHandTCP`、`/World/Visuals/DrawerHandleTop`，保留 `/World/Visuals/TargetBlockTCP`。新增 `--show-drawer-handle-frame` 和 `--drawer-handle-frame-prim`。左/右 TCP 都从对应 `left_wrist_yaw_link/right_wrist_yaw_link` 加 `(0,0,-0.10)m` offset 估计。抽屉 static preview 现在也支持 `--keyboard-jog`，并写入/监听 `/tmp/s4_arm_control.json`，所以可用 `bash run.sh control test-right-arm ...` 在运行中的抽屉场景里发送可复现的右臂 7D 关节目标。推荐调试命令：`bash run.sh sim --print-layout --show-tcp-frames --show-drawer-handle-frame --print-tcp-pose --keyboard-jog`。
142. 2026-07-31 按用户最新反馈修正抽屉任务调试框架。第一，`drawer_handle_frame` 是坐标系，不是 visual handle；`scripts/03_record_physics_dataset.py` 的 `--drawer-handle-frame-prim` 默认已改为 `/World/DrawerTask/DrawerCabinet/drawer_handle_frame`，fallback 也按 leaf name `drawer_handle_frame` 搜索，避免再次显示 `/drawer_top/drawer_handle_top` 这类 visual/handle prim。第二，`tasks/drawer_insert_close_scene.py` 的抽屉位置改为 `(1.20, 0.00, 0.70)`，TomatoSoupCan 显式 root 位置改为 `(0.57, -0.20, 1.16)`，不再用 bbox 自动对齐覆盖用户给定坐标。第三，新增 `s4_robot/pink_bimanual_ik.py`，使用 IsaacLab 自带 `PinkIKController` 和已安装的 `pink/qpsolvers/daqp/pinocchio`，以 URDF `base_link` 为求解坐标系、以 `left_wrist_yaw_link/right_wrist_yaw_link` 为 frame task，输出左右臂 14D 关节目标。第四，`scripts/control_arm.py` 新增 `test-left-arm`、`test-bimanual-arm`、`tcp-pose`；抽屉 static preview 已能监听这些命令，并在收到 arm-control 命令后主动 step physics，让目标实际执行。`tcp-pose` 的 `--left-pos/--right-pos` 和 `--left-rpy/--right-rpy` 均解释为 robot `base_link` 下目标位姿；`--print-tcp-pose` 现在同时打印 world 和 base_link 下的 TCP/handle pose，避免用 world 坐标去喂 base_link IK。注意：该 Pink IK 接口当前用于 drawer 静态预览/调试手臂位姿，尚未接入正式 drawer 数据采集控制器。
143. 2026-07-31 用户反馈发送关节角后只有 TCP marker 在动、手臂不动，发送笛卡尔位姿没反应且没有调试信息。排查后修正三点：第一，`tcp-pose` 不能只调用 Pink IK 一次，因为 Pink 返回的是小步 joint target；现在抽屉 static preview 在 `tcp_pose_active=True` 时每个仿真 step 持续用当前关节状态求解并更新左右臂目标。第二，命令语义是 TCP，而 Pink frame task 控制的是 URDF wrist frame；`s4_robot/pink_bimanual_ik.py` 现在会用 `DEFAULT_TCP_OFFSET_WRIST=(0,0,-0.10)m` 把 base_link 下 TCP 目标反算成 wrist frame 目标。第三，`run.sh control` 原来调用 `python`，若当前 shell 没有 `python` 会导致控制文件不写入；已改为 `python3`。同时新增 `[ARMDBG] active=... tcp_pose=... left_q_max_err=... right_q_max_err=...` 周期日志，并在 Pink setup/continuous solve 失败时打印 traceback，后续不要再静默失败。
144. 2026-07-31 用户运行 `tcp-pose` 后 Pink setup 报错：`No Python class registered for C++ class std::vector<std::string>`，栈在 IsaacLab 的 `PinkKinematicsConfiguration.__init__ -> self.full_model.names.tolist()`。这说明 IsaacSim 进程中的 Pinocchio/Boost Python 绑定与 IsaacLab Pink wrapper 对 `model.names.tolist()` 的假设不兼容。已重写 `s4_robot/pink_bimanual_ik.py`：不再导入 `isaaclab.controllers.pink_ik.PinkIKController`，改为直接使用 `pink.Configuration`、`pink.solve_ik`、`pink.tasks.FrameTask/DampingTask` 和 Pinocchio reduced model；所有关节名均用 `list(model.names)` 读取，绕开 `.tolist()`。模块启动时会优先把 cmeel Pinocchio site-packages 放到 `sys.path` 前面，并打印 `[PINK] direct solver ready ... controlled_pin_order=...`。本地检查已确认 cmeel Pinocchio 可加载 URDF，`left_wrist_yaw_link/right_wrist_yaw_link` frame 存在。
145. 2026-07-31 用户再次报错，同样的 `std::vector<std::string>` 绑定问题出现在 `list(self.full_model.names)`，说明在 IsaacSim 进程里任何访问 `model.names` 都不安全。已继续重写 `s4_robot/pink_bimanual_ik.py`：完全不读取 `model.names`，改为从 URDF XML 解析所有非 fixed joint 名称，再通过 Pinocchio `model.getJointId(name)` 和 `model.idx_qs[joint_id]` 建立 `URDF joint name -> q index` 映射；reduced model 输出顺序也用 `reduced_model.getJointId/idx_qs`，彻底绕开 names vector。随后本地 sanity test 暴露 `qpsolvers + daqp` 版本不兼容：`daqp.solve() got unexpected keyword argument 'primal_start'`。当前可用 solver 为 `['daqp', 'quadprog']`，已把 Pink solver 改为 `quadprog`。本地测试通过：构造 fake robot joint_names、实例化 `PinkBimanualTcpController`、对右手 `tcp-pose` 调用一次 `compute()`，输出 14D 中左臂保持 0、右臂产生非零目标。
146. 2026-07-31 用户继续报错：`No Python class registered for C++ class std::vector<int>`，位置在 `self.full_model.idx_qs[joint_id]`。这确认 IsaacSim 进程里 Pinocchio 的 C++ vector 绑定整体都不能安全访问，不只是 string vector。已再次修改 `s4_robot/pink_bimanual_ik.py`：不再访问 `model.idx_qs` / `reduced_model.idx_qs`。因为当前 S4 URDF 是固定基座，所有非 fixed joints 都是 1DOF，Pinocchio q 顺序等于 URDF 非 fixed joint 顺序；因此 `pin_name_to_q_index` 直接用 `enumerate(movable_urdf_joint_names())`，reduced model q 顺序直接用原 URDF 顺序中过滤出的 controlled joints 枚举。现在源码里没有任何实际 `.names`、`.idx_qs`、`.tolist()` 访问；本地 sanity test 仍通过，右手 `tcp-pose` 能输出非零右臂 7D 目标。
147. 2026-07-31 用户继续报错：`No to_python converter for std::vector<bool>`，位置在 `pink.Configuration -> ConfigurationLimit(model) -> model.hasConfigurationLimit()`。这确认当前 IsaacSim 进程中不只是 IsaacLab Pink wrapper，Pink 自身也不能用，因为它内部会调用返回 C++ vector 的 Pinocchio API。已把 `s4_robot/pink_bimanual_ik.py` 内部彻底改为 Pinocchio DLS fallback：不再 import/use `pink.Configuration`、`pink.solve_ik`、`FrameTask`、`DampingTask`；只用 Pinocchio `computeJointJacobians`、`updateFramePlacements`、`getFrameJacobian` 和 `data.oMf[frame_id]`，然后用 NumPy 阻尼最小二乘求 `dq = J.T (J J.T + λ²I)^-1 e`。随后发现“URDF 非 fixed 顺序就是 q 顺序”也不对，右臂真实 q 列在 30-36；已改用安全标量 API `full_model.getJointId(name) - 1` 建 q index，不访问 `idx_qs`。本地检查：受控 q index 为 `[6..12, 30..36]`，右 wrist Jacobian 右臂列范数非零，右手 `tcp-pose` 输出右臂 7D 非零关节目标。
148. 2026-07-31 用户反馈 drawer 调试中机器人“软绵绵”，怀疑没有重力补偿。确认属实：蓝色圆柱抓取主循环一直调用 `apply_gravity_compensation()`，但后来新增的 `run_static_task_scene()` drawer/static preview 漏掉了该调用。已修复：drawer preview 初始化 `gravity_comp_joint_ids = ALL_DRIVE_JOINTS` 中存在的关节；每轮在 `robot.set_joint_position_target()` 后、`robot.write_data_to_sim()` 前调用同一套 `apply_gravity_compensation(robot, joint_ids, --gravity-comp-scale, --gravity-compensation)`。启动时打印 `[SCENE] drawer preview gravity_compensation=True scale=... joints=...`，`[ARMDBG]` 日志追加 `gravity_comp=max/mean`。若仍偏软，优先调 `--joint-stiffness/--joint-damping/--joint-effort-limit` 和 `--gravity-comp-scale`，不要误判为 IK 无输出。
149. 2026-07-31 按用户要求给抽屉 TCP IK 加入 null-space posture bias，用于让手肘保持外侧、远离身体。实现位置：`s4_robot/pink_bimanual_ik.py` 的 Pinocchio DLS fallback。主任务仍是 TCP pose DLS：`dq_task = J# e`，新增项为 `dq = dq_task + (I - J#J) * k * (q_home - q)`，其中 `q_home` 来自 `s4_robot_cfg.DEFAULT_POSE` 的左右臂 14D 默认姿态，`k` 默认 `--tcp-posture-gain 0.30`。同时新增 `--tcp-ik-damping` 和 `--tcp-max-joint-delta` 便于调试。注意：该 bias 是冗余空间偏置，不是碰撞检测；它能降低肘部内收倾向，但不能保证不撞身体。若 TCP 目标跟踪变差，先降 `--tcp-posture-gain 0.10` 或 `0.05`。README 与 `docs/TASKS.md` 已同步说明“当前不是 Pink 原生求解器，而是 Pinocchio DLS fallback + null-space posture”。
150. 2026-07-31 按用户要求在抽屉任务里再导入一个同款 Sektion cabinet，用于把两个抽屉拼到一起。实现位置：`tasks/drawer_insert_close_scene.py`。保留主抽屉 prim `/World/DrawerTask/DrawerCabinet`，新增第二个 prim `/World/DrawerTask/DrawerCabinetSecondary`。两个抽屉共用同一 `sektion_cabinet_instanceable.usd`、同一 yaw 姿态和同一高度 `z=0.70`，x 对齐为 `x=0.80`，只在 y 方向偏移：主抽屉 `y=+0.30`，第二抽屉 `y=-0.30`。后续 drawer handle 可视化和 drawer drive 默认仍指向主抽屉 `/World/DrawerTask/DrawerCabinet/drawer_handle_frame`；如果要调第二个抽屉，需要把 `--drawer-handle-frame-prim` 或 joint filter/prim path 指向 `DrawerCabinetSecondary` 下对应 frame/joint。
151. 2026-07-31 用户要求“把关节偏好的比例从默认 0.03 调到 0.3，并把现在启动的参数写成默认值”。已统一四处默认值：`s4_robot/arm_control.py::DEFAULT_POSTURE_GAIN = 0.30`，`scripts/03_record_physics_dataset.py --reach-posture-gain default=0.30`，`scripts/03_record_physics_dataset.py --tcp-posture-gain default=0.30`，以及 `s4_robot/pink_bimanual_ik.py` 构造函数默认 `posture_gain=0.30`。当前抽屉 TCP 调试推荐命令不需要显式传 gain：`bash run.sh sim --print-layout --show-tcp-frames --show-drawer-handle-frame --print-tcp-pose`；等价显式参数为 `--tcp-posture-gain 0.30 --tcp-ik-damping 0.08 --tcp-max-joint-delta 0.025`。如果出现 TCP 跟踪明显变差或卡住，再临时降为 `--tcp-posture-gain 0.10` 或 `0.05`。
